import { Router } from "express";
import type { Bucket, BucketReframe } from "../content/bucketReframes.js";
import {
  createSortedThought,
  getThoughtById,
  recordOutcome,
  searchThoughts,
} from "../services/thoughtService.js";
import { asyncHandler } from "../middleware/asyncHandler.js";

const VALID_BUCKETS: Bucket[] = ["full_control", "partial_control", "no_control"];

export const thoughtsRouter = Router();

thoughtsRouter.get(
  "/",
  asyncHandler(async (req, res) => {
    const { query, bucket, from, to } = req.query as {
      query?: string;
      bucket?: Bucket;
      from?: string;
      to?: string;
    };
    res.json(await searchThoughts({ query, bucket, from, to }));
  })
);

thoughtsRouter.get(
  "/:id",
  asyncHandler(async (req, res) => {
    const thought = await getThoughtById(Number(req.params.id));
    if (!thought) {
      res.status(404).json({ error: "Sorted thought not found." });
      return;
    }
    res.json(thought);
  })
);

thoughtsRouter.post(
  "/",
  asyncHandler(async (req, res) => {
    const { entry_date, worry_text, bucket, custom_reframe } = req.body as {
      entry_date?: string;
      worry_text?: string;
      bucket?: Bucket;
      /** Present when saving an AI-assist suggestion (Step 7) — omit for the static baseline. */
      custom_reframe?: BucketReframe;
    };

    if (!entry_date || !worry_text?.trim() || !bucket) {
      res.status(400).json({ error: "entry_date, worry_text, and bucket are required." });
      return;
    }
    if (!VALID_BUCKETS.includes(bucket)) {
      res.status(400).json({ error: `bucket must be one of: ${VALID_BUCKETS.join(", ")}` });
      return;
    }

    const thought = await createSortedThought({
      entryDate: entry_date,
      worryText: worry_text,
      bucket,
      customReframe: custom_reframe,
    });
    res.status(201).json(thought);
  })
);

thoughtsRouter.put(
  "/:id/outcome",
  asyncHandler(async (req, res) => {
    const { outcome_note } = req.body as { outcome_note?: string };
    if (!outcome_note?.trim()) {
      res.status(400).json({ error: "outcome_note is required." });
      return;
    }
    const thought = await recordOutcome(Number(req.params.id), outcome_note);
    if (!thought) {
      res.status(404).json({ error: "Sorted thought not found." });
      return;
    }
    res.json(thought);
  })
);
